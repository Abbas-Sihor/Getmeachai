...
//   }, [session, router]);
// 	return (
// 		<div className="mx-auto flex flex-col p-5 justify-center items-center">
// 			<h2 className="text-center font-bold text-2xl text-[#fffbe1]">Login To Continue</h2>

// 			<div className="flex flex-col gap-2 p-10">
// 				<button onClick={() => signIn("google")} className="flex items-center bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
// 					<svg
  ...
  return (
    <div className="relative min-h-screen overflow-hidden  text-white">
      ...
      {/* Background Glow */}
        ...
        bg-[radial-gradient(circle_at_center,rgba(0,153,255,0.25),transparent_60%)]
        before:absolute before:inset-0 
        before:bg-[repeating-radial-gradient(circle,rgba(0,255,255,0.05)_0px,rgba(0,255,255,0.05)_1px,transparent_1px,transparent_6px)]
        before:content-['']"
      /> */}
     

      {/* Main */}
      <main className="relative  grid min-h-[70vh] grid-cols-1 gap-16 px-10 py-16 lg:grid-cols-2 lg:px-20">
        
        {/* Left */}
        <div>
          <h2 className="text-5xl font-extrabold leading-tight lg:text-6xl">
            UNLOCK YOUR <br />
            <span className="text-sky-400">IT POTENTIAL</span>
          </h2>

          <p className="mt-4 text-lg text-white/80">
            Seamless Access. Smarter Solutions.
          </p>

          {/* Buttons */}
          <div className="mt-10 flex max-w-sm flex-col gap-4">
            <button className="flex items-center gap-3 rounded-lg bg-[#4285F4] px-6 py-4 text-lg font-medium transition hover:-translate-y-0.5 hover:shadow-xl" onClick={()=>signIn("google")}>
							
              <Image src="/google.svg" className="rounded-full" alt="Google" width={22} height={22} />
              Login with Google
            </button>

            <button className="flex items-center gap-3 rounded-lg bg-[#111827] px-6 py-4 text-lg font-medium transition hover:-translate-y-0.5 hover:shadow-xl" onClick={()=>signIn("github")}>
              <Image src="/github.svg" alt="GitHub" width={22} height={22} className="text-white" />
              Login with GitHub
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center">
          <div className="relative max-w-md border border-sky-400/40 bg-black/30 p-8 backdrop-blur">
            <p className="text-xl leading-relaxed">
              “Join a community of innovators.
              <br />
              Streamline your gain insights.
              <br />
            ...
            {/* Quote Marks */}
            <span className="absolute -left-4 -top-4 text-6xl text-sky-400">“</span>
            <span className="absolute -bottom-6 -right-4 text-6xl text-sky-400">”</span>
          </div>
        </div>
      </main>

      {/* Footer */}
      <footer className="relative z-10 px-10 pb-6 text-xs text-white/70 lg:px-20">
        <p>
          <span className="font-semibold">BENEFITS:</span> Personalized Dashboard • Project Collaboration • Advanced Analytics • Cloud Support • Exclusive Content
        </p>
        <p className="mt-1">
          <span className="font-semibold">WHAT’S AHEAD:</span> AI-Powered Project Management • Predictive Maintenance • Secure Data • Global IT Network Access
        </p>
      </footer>
    </div>
  );
